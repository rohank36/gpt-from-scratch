...
# All unique chars in the input text
...
# Split tokenized dataset -> train and validation 
data = torch.tensor(encode(text), dtype=torch.long)
n = int(0.9*len(data)) 
train_data = data[:n]
val_data = data[n:]

# Generate batch of data (inputs and targets) from the training dataset or validation dataset 
def get_batch(split):
...
m = model.to(device)
optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate) # Create pytorch optimizer 
